...
void WiFiSettingsClass::portal() {
    ...
    unsigned long starttime = millis(); //mano
    ...
    for (;;) {
        ...
        //esp_task_wdt_reset();
        delay(1);
        if ((millis() - starttime) > 240000){ESP.restart();} //Perstartuojam ESP ir bando prisijungti prie seno WIFI jei neprisijunge pries tai [ms] ir naujas neivedamas
    }
}
...
int WiFiSettingsClass::status(){
    return WiFi.status();
}

String WiFiSettingsClass::macAdress(){
    return ESPMAC;
}
